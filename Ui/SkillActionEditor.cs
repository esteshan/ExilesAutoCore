...
/// <summary>
/// Edits a <see cref="SkillAction"/>: the input kind (key, left-click, right-click), the key
/// picker when kind is Key, and the optional auto-face target filter. Shared by the rule builder
/// and the combo-step builder.
/// </summary>
public static class SkillActionEditor
{
    public static void Draw(SkillAction action)
    {
        ...
        Controls.EnumCombo("##inputkind", ref action.InputKind);

        if (action.InputKind == SkillInputKind.Key)
        {
            ImGui.SameLine();
